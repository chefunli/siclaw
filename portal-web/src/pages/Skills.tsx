...
const STATUS_STYLES: Record<string, { bg: string; text: string; labelKey: string }> = {
  draft: { bg: "bg-yellow-500/20", text: "text-yellow-400", labelKey: "skills.draft" },
  pending_review: { bg: "bg-blue-500/20", text: "text-blue-400", labelKey: "skills.pendingReview" },
  installed: { bg: "bg-green-500/20", text: "text-green-400", labelKey: "skills.installed" },
}
...
export function Skills() {
  ...
  // ── Actions ───────────────────────────────────────────────────
    ...
    if (!(await confirmDialog({
      title: t("skills.deleteTitle") || "Delete Skill", message: t("skills.deleteMessage") || "Delete this skill and all versions? This cannot be undone.",
          ...
          <div className="flex items-center gap-2">
            {isAdmin && (
              <button onClick={() => navigate("/skills/import")} className="flex items-center gap-1.5 h-8 px-3 text-sm rounded-md border border-border text-muted-foreground hover:text-foreground">
                <Upload className="h-3.5 w-3.5" /> {t("skills.importSkill")}
              </button>
            )}
            <button onClick={() => navigate("/skills/new")}
              ...
              <Plus className="h-3.5 w-3.5" /> {t("skills.newSkill")}
            </button>
          </div>
        )}
      </div>
              ...
              {labelDropdownOpen && (
                <div className="absolute right-0 top-full mt-1 w-56 bg-card rounded-lg shadow-lg border border-border z-30 py-1 max-h-72 overflow-y-auto">
                  {availableLabels.length === 0 ? (
                    <p className="px-3 py-2 text-[11px] text-muted-foreground">{t("skills.noLabels") || "No labels found"}</p>
        ...
        {activeTab === "all" ? (
          ...
          loading && skills.length === 0 ? (
            ...
            <div className="px-6 py-4 space-y-2">
              {skills.map(s => {
                                  ...
                                  <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                      <div className="flex items-center gap-2">
                                        ...
                                        <span className={`px-1.5 py-0.5 rounded text-[10px] ${st.bg} ${st.text}`}>{t(st.labelKey)}</span>
                                        {!!s.is_builtin && (
                                          <span className="px-1.5 py-0.5 rounded text-[10px] bg-purple-500/20 text-purple-400">{t("skills.builtin")}</span>
                                        )}
                                        {s.overlay_of && (
                                          <span className="px-1.5 py-0.5 rounded text-[10px] bg-cyan-500/20 text-cyan-400">{t("skills.overlay")}</span>
                                        )}
                                        {s.installed_version != null && s.installed_version > 0 && (
                                          <span className="text-[10px] text-green-400">{t("skills.installedVersion", { 0: s.installed_version })}</span>
                                        )}
                                        {s.version !== (s.installed_version ?? 0) && (
                                          <span className="text-[10px] text-muted-foreground">{t("skills.draftVersion", { 0: s.version })}</span>
                                        )}
                                      </div>
                                      <p className="text-xs text-muted-foreground truncate">{s.description || t("skills.noDescription")}</p>
                        {lbls.length > 0 && (
                      ...
                      <button onClick={() => navigate(`/skills/${s.id}`)} title={t("common.edit")}
                        className="p-1.5 rounded-md hover:bg-secondary text-muted-foreground hover:text-foreground">
                        <Pencil className="h-4 w-4" />
                      </button>
                      {!s.is_builtin && (
                        <button onClick={() => handleDelete(s.id)} title={t("common.delete")}
          ...
          reviewsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20">
              ...
              <p className="text-sm text-muted-foreground">{t("skills.noPendingReviews") || "No pending reviews"}</p>
            </div>
          ) : (
      ...
      {activeTab === "all" && totalPages > 1 && (
        <div className="flex items-center justify-center gap-2 px-6 py-3 border-t border-border">
          <button disabled={page <= 1} onClick={() => setPage(p => p - 1)}
            className="h-7 px-3 text-xs rounded border border-border disabled:opacity-50">{t("common.prev") || "Prev"}</button>
          <span className="text-xs text-muted-foreground">{t("common.page") || "Page"} {page} {t("common.of") || "of"} {totalPages}</span>
          <button disabled={page >= totalPages} onClick={() => setPage(p => p + 1)}
            className="h-7 px-3 text-xs rounded border border-border disabled:opacity-50">{t("common.next") || "Next"}</button>
        </div>
      )}
    </div>
  )
...
function ReviewApprovalCard({ review: r, onDecision }: { review: PendingReview; onDecision: () => void }) {
  ...
  const handleApprove = async () => {
      if (!(await confirmDialog({
        title: t("skills.approveSkill"),
        message: t("skills.approveConfirmMessage", { 0: r.skill_name }),
        confirmLabel: t("skills.approve"),
      }))) return
      setSubmitting(true)
      try {
        await api(`/siclaw/skills/${r.skill_id}/approve`, { method: "POST" })
        toast.success(t("skills.skillApproved", { 0: r.skill_name }))
        onDecision()
      } catch (err: any) { toast.error(err.message) }
    ...
    const handleReject = async () => {
      if (!(await confirmDialog({
        title: t("skills.rejectSkill"),
        message: t("skills.rejectConfirmMessage", { 0: r.skill_name }),
      ...
      try {
        ...
        toast.success(t("skills.skillRejected", { 0: r.skill_name }))
        onDecision()
      } catch (err: any) { toast.error(err.message) }
      finally { setSubmitting(false) }
    }
          ...
          <div className="relative w-full max-w-3xl bg-card rounded-xl shadow-xl border border-border overflow-hidden flex flex-col max-h-[85vh]">
                ...
                <>
                  ...
                  {diffData?.comment && (
                    <div className="px-3 py-2 rounded-md bg-secondary/50 text-[12px]">
                                          <span className="font-medium text-muted-foreground">{t("skills.comment")}</span> {diffData.comment}
                                        </div>
                  )}
                  ...
                  {!sa && (
                    <div className="rounded-lg border border-border p-4 flex items-center justify-center gap-2">
                      ...
                      <span className="text-[12px] text-muted-foreground">{t("skills.securityAssessment")}</span>
                    </div>
                  )}
                  {sa && (
                    <div className="rounded-lg border border-border overflow-hidden">
                      <div className="px-4 py-3 bg-secondary/30 flex items-center gap-3">
                        <span className="text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">{t("skills.securityReview")}</span>
                        {(() => {
                      ...
                      <div className="p-4 space-y-3">
                        ...
                        {(!sa.findings || sa.findings.length === 0) && (
                          <div className="text-center py-3 text-[12px] text-green-400">{t("skills.noSecurityIssues")}</div>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  {reviewDetail?.diff && (
                    <div className="space-y-2">
                      <span className="text-[12px] font-medium text-muted-foreground">{t("skills.changes")}</span>
            ...
            <div className="border-t border-border px-6 py-3 space-y-2 shrink-0">
              <input value={reason} onChange={e => setReason(e.target.value)}
                              placeholder={t("skills.rejectReason")}
              ...
              <div className="flex items-center justify-end gap-2">
                <button onClick={() => setExpanded(false)} className="h-8 px-3 text-[12px] text-muted-foreground hover:text-foreground">{t("common.cancel")}</button>
                                <button onClick={handleReject} disabled={submitting}
                                  ...
                                  <XCircle className="h-3.5 w-3.5" /> {t("skills.reject")}
                                </button>
                                <button onClick={handleApprove} disabled={submitting}
                                  ...
                                  <CheckCircle className="h-3.5 w-3.5" /> {t("skills.approve")}
                                </button>
              </div>
            </div>
          </div>
